...
class MaskTwoStreamConv_NET(MaskContextAE_NET):
    def __init__(self, opt):
        super(MaskTwoStreamConv_NET, self).__init__(opt)
        ...
        #self.dim_list_dec = [256, 128, 96, self.conv_dim, self.conv_dim/2] 
    ...
    def get_conv_decoder(self, output_nc, skip_layers=None):
        ...
        for i in range(self.num_layers+1):
            ...
            #input_dim = self.dim_list[i-self.num_layers-1]
            #output_dim = self.dim_list[i-self.num_layers]
            input_dim = output_dim
    ...
    def forward(self, input_var, cls_onehot, is_bkg=False):
        ...
        # 4. combine outputs from both branches
        # TODO(sh): selectively turn on and off the padded mask 
        #           to be zero (if it is either from background or there is no obj)
        comb_output_logit, comb_output_prob = None, None
        if ('context' in self.which_stream) and not ('obj' in self.which_stream):
            comb_output_logit = ctx_output_logit
            comb_output_prob = ctx_output_prob
        elif ('context' in self.which_stream) and ('obj' in self.which_stream):
            padded_mask = obj_output_prob.expand_as(ctx_output_prob) # padded mask
            B, C, H, W = ctx_output_logit.size()
            padded_obj_output = torch.bmm(cls_onehot.view(B, C, 1), \
                                          obj_output_logit.view(B, 1, H * W))
            padded_obj_output = padded_obj_output.view(B, C, H, W)
            comb_output_logit = (1 - padded_mask) * ctx_output_logit + \
                    padded_mask * obj_output_logit
            comb_output_prob = self.log_softmax(comb_output_logit)

        return comb_output_logit, comb_output_prob, obj_output_logit, obj_output_prob
